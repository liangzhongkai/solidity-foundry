...
contract UniswapV3LiquidityNftExample {
    ...
    function mintPosition(
        ...
        _refundToken(token0, msg.sender, amount0Desired - a0);
        _refundToken(token1, msg.sender, amount1Desired - a1);
    ...
    function _clearApproval(address token) internal {
        IERC20(token).forceApprove(address(positionManager), 0);
    }

    function _refundToken(address token, address recipient, uint256 amount) internal {
        if (amount > 0) {
    ...
    function _requirePositionOwner(uint256 tokenId) internal view {
        if (IERC721(address(positionManager)).ownerOf(tokenId) != msg.sender) revert UnauthorizedPositionCaller();
    }
