...
contract UniswapV4PositionManagerExample {
    ...
    mapping(uint256 => address) public positionController;
    ...
    function _requirePositionController(uint256 tokenId) internal view {
        address controller = positionController[tokenId];
        if (controller != address(0)) {
            if (msg.sender != controller) revert UnauthorizedPositionCaller();
            return;
        }

        if (IERC721PositionAuth(address(positionManager)).ownerOf(tokenId) != msg.sender) {
